...
class PostDB{
    ...
    async removePost(post_id: string, id_user: string) {
        try{
            const data = await this.post.deleteOne({ _id: post_id, id_creator: id_user });
            return data;
        } catch (err: any) {
            throw err;
        }
    ...
    async removeLike(post_id: string, id: string) {
        try{
            const data = await this.post.updateOne({ _id: post_id },{$pull:{likes:id} });
            return data;
        } catch (err: any) {
            throw err;
        }
    ...
    async addLike(post_id: string, id: string) {
        try{
            const data = await this.post.updateOne({ _id: post_id },{$push:{likes:id} });
            return data;
        } catch (err: any) {
            throw err;
        }
    ...
    async addComment(id: string,comment: IComment) {
        try{
            const data = await this.post.updateOne({ _id: id },{$push:{comments:comment} });
            return data;
        } catch (err: any) {
            throw err;
        }
